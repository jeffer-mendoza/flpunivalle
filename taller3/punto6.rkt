...
(scan&parse "class Cola extends object
field lista

method initialize () set lista = list(1)

method empty()
if null?(lista) then 1 else 0


method encolar_aux(colalista,x)
if null?(colalista)
     then x
     else cons(car(colalista), encolar_aux(cdr(colalista), x))

method encolar(x)
begin
   set lista=encolar_aux(lista,x)
end
...
in
   let
      a = send cola empty()
      b = send cola encolar(2)
      c = send cola encolar(3)
      d = send cola desencolar()
   in
      send cola getLista()
